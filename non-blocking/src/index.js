...
const setupShutdownHandler = (server, monitor) => {
  const shutdown = async (signal) => {
    ...
    if (stats.blockingEvents === 0) {
      console.log(`   ✨ Perfect! No blocking detected!`);
    }
...
const main = async () => {
  console.log('╔════════════════════════════════════════════════════════════════╗');
  console.log('║   NON-BLOCKING DEMO - Async Best Practices                    ║');
  ...
  try {
    ...
    console.log('💡 Tip: Compare with blocking app using load testing');
    console.log('   Example: autocannon http://localhost:3001/compute-async -d 10 -c 5\n');
